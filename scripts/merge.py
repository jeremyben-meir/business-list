...
def load_source_files():
    df_o1 = pickle.load( open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/dca/temp/df-insp.p", "rb" ))
    df_o2 = pickle.load( open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/dca/temp/df-app-1.p", "rb" ))
    df_o3 = pickle.load( open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/dca/temp/df-charge-1.p", "rb" ))
    df_o4 = pickle.load( open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/doh/temp/df-doh-2.p", "rb" ))
...
def begin_process(segment):
    ...
    if 0 in segment:
        ...
        pickle.dump(df, open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/temp/df-1.p", "wb" ))
        print("load done")

    if 1 in segment:
        ## ADD LLID ON BBL
        df = pickle.load( open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/temp/df-1.p", "rb" ))
        df = add_llid(df, bbl=True)
        pickle.dump(df, open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/temp/df-2.p", "wb" ))
        print("added LLIDs on BBL")

    if 2 in segment:
        ## ADD LLID ON ADDRESS
        df = pickle.load( open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/temp/df-2.p", "rb" ))
        df = add_llid(df, bbl=False)
        pickle.dump(df, open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/temp/df-3.p", "wb" ))
        print("added LLIDs on address")

    if 3 in segment:
        ## ADD LBID
        df = pickle.load( open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/temp/df-3.p", "rb" ))
        df = df[df['LLID'].str.len() > 0]
        df = df[df['Record ID'].str.len() > 5]
        df = add_lbid(df)
        pickle.dump(df, open(f"{DirectoryFields.LOCAL_LOCUS_PATH}data/temp/df-4.p", "wb" ))
        print("added LBIDS")
