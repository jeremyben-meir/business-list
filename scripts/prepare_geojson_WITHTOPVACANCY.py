...
class PrepareGeojson():
    ...
    def create_bbl_json_2(self):
        ...
        df["block"] = df["BBL"].apply(lambda bbl: bbl[:-4])
        print(df["block"])
        ...
        for name, group in grouped:
            subgrouped = group.groupby("BBL")
            sumVacancy = 0
            for subname, subgroup in subgrouped:
                latitude, longitude = map(float, (subgroup["Latitude"].max(), subgroup["Longitude"].max()))
                ## VACANCY
                max_llid = int(subgroup['Year'].value_counts().max())
                vacancy = round(float((len(subgroup) / len(self.year_list)) / max_llid),2)
                sumVacancy += vacancy
            ...
            if (len(subgrouped)>5 and name[0]=="1"):
                mostVacantDict[name] = sumVacancy/len(subgrouped)

        vacantBlocks = sorted(mostVacantDict, key=mostVacantDict.get, reverse=True)[:5]
        print(vacantBlocks)
        print([round(mostVacantDict[blk],5) for blk in vacantBlocks])
