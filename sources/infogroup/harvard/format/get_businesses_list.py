...
manhattanZipsStr = [str(x) for x in manhattanZips]
nonManhattanZipsStr = [str(x) for x in nonManhattanZips]

totalZips = manhattanZips + nonManhattanZips

for year in YEAR_LIST:

    print(year)

    business_file_path = LOCAL_LOCUS_PATH + "data/"+year+"/sources/primary/wholeBusinessList.txt"
    cleaned_business_file_path = LOCAL_LOCUS_PATH + "data/"+year+"/sources/NYBusinessList.csv"

    df_business = pd.read_csv(business_file_path, encoding="ISO-8859-1")
    df_business['ZipCode'] = df_business['ZipCode'].astype(int)
    df_business_nyc = df_business.loc[df_business['ZipCode'].isin(totalZips)]

    df_business_nyc.to_csv(cleaned_business_file_path, index=False, quoting=csv.QUOTE_NONNUMERIC)